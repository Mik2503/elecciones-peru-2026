...
function CorrupcionSection() {
  ...
  const indicators = [
    { id: "material", title: "Material Electoral No Entregado", severity: 78, status: "CONFIRMADO", evidence: "63,300 personas no pudieron votar por falta de material electoral en Lima. Defensoría del Pueblo emitió alerta oficial.", source: "Defensoría del Pueblo, 12 abril 2026" },
    { id: "noflash", title: "Eliminación del Flash Electoral", severity: 65, status: "CONFIRMADO", evidence: "ONPE eliminó el tradicional flash electoral, reduciendo transparencia y escrutinio público en tiempo real.", source: "Declaración ONPE, Piero Corvetto" },
    { id: "votos", title: "Denuncias de Compra de Votos", severity: 85, status: "EN INVESTIGACIÓN", evidence: "JNE emitió Resolución Nº 0393-2026-JNE sobre compra de votos y fraude electoral. Múltiples denuncias bajo investigación.", source: "JNE Resolución 0393-2026-JNE" },
    { id: "exterior", title: "Voto Suspendido en Medio Oriente", severity: 72, status: "CONFIRMADO", evidence: "Canciller anunció que elecciones no se realizarán en Medio Oriente por 'falta de condiciones de seguridad'.", source: "TVPerú Noticias, marzo 2026" },
    { id: "cedulas", title: "Costo Millonario Traslado Cédulas", severity: 55, status: "CONFIRMADO", evidence: "Cédulas del extranjero vuelven a Lima costando S/1M+. Antes se custodiaban localmente.", source: "Ojo Público #6159" },
    { id: "locales", title: "ONPE 'Mendigó' Locales", severity: 60, status: "CONFIRMADO", evidence: "Jefe ONPE reveló que instituciones se negaron a ceder espacios, comprometiendo infraestructura electoral.", source: "El Comercio, marzo 2026" },
    { id: "observers", title: "Observación Internacional Activa", severity: -15, status: "POSITIVO", evidence: "OEA y UE tienen misiones de observación activas, reduciendo riesgo de fraude.", source: "ANDINA, marzo 2026" },
    { id: "fragmentacion", title: "Fragmentación Electoral Histórica", severity: 45, status: "DATO", evidence: "36 candidatos presidenciales. Solo 28% votantes bien informados.", source: "Ipsos, CPI, Datum" },
  ];
  const score = 62;
  return (
    <div className="space-y-4">
        ...
        <div key={ind.id} className="bg-zinc-900/40 rounded-xl border border-zinc-800 overflow-hidden">
          <button onClick={() => setExpanded(expanded === ind.id ? null : ind.id)} className="w-full p-4 flex items-center justify-between text-left hover:bg-zinc-800/30">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-semibold text-white text-sm">{ind.title}</p><p className="text-[10px] text-zinc-500">{ind.status}</p></div>
            </div>
          ...
          {expanded === ind.id && (
            <div className="p-4 border-t border-zinc-800 space-y-2 bg-zinc-900/60">
              <p className="text-zinc-300 text-sm">{ind.evidence}</p>
